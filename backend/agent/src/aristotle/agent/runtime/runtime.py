...
:mod:`aristotle.agent.context.assembly`, the model call to a
:class:`~aristotle.agent.model.protocol.SendsMessages` implementation, tool execution
to :mod:`aristotle.agent.tools.execution`, and every state write to a
:class:`~aristotle.agent.session.protocol.SessionRepository`. It reimplements none of
them.
...
from aristotle.agent.context.assembly import (
    DEFAULT_MEMORY_LIMIT,
    AssembledContext,
    assemble_context,
)
from aristotle.agent.context.retrieval import SuppliesMemoryCandidates
from aristotle.agent.model.protocol import ModelResponse, SendsMessages, ToolCall
from aristotle.agent.session.protocol import SessionRepository
from aristotle.agent.session.store import TranscriptItem
from aristotle.agent.tools.execution import (
    Approve,
    ToolExecutionError,
    ToolResult,
    execute_tool_call,
)
from aristotle.agent.tools.registry import ToolRegistry
...
class Runtime:
    ...
    Args:
        model_client: Anything satisfying
            :class:`~aristotle.agent.model.protocol.SendsMessages`. The runtime is
        ...
        session_store: Anything satisfying
            :class:`~aristotle.agent.session.protocol.SessionRepository`. The runtime
    ...
    async def run_turn(
        ...
        Args:
            ...
            approve: Gate for each proposed call. Omitting it allows everything,
                ...
                :func:`aristotle.agent.tools.approval.cli_approve`.
