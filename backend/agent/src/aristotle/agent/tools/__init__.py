...
- :mod:`~aristotle.agent.tools.registry` — what tools exist, and which the model is
  told about this run.
- :mod:`~aristotle.agent.tools.approval` — whether this specific call, with these
  arguments, should happen now.
- :mod:`~aristotle.agent.tools.execution` — actually running it, once and only once
...
:mod:`~aristotle.agent.tools.notes` is a worked example — copy its shape when adding a
tool.
"""
