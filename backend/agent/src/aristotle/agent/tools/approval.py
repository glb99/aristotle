...
- **isolation** — if it runs and goes wrong, how far does the damage reach?
  ...
  :mod:`aristotle.agent.tools.execution`.
...
Not built:
    ...
    :data:`aristotle.agent.tools.execution.Approve` accepts a coroutine function, so a
...
from aristotle.agent.model.protocol import ToolCall
...
def cli_approve(tool_call: ToolCall, input_fn: Callable[[str], str] = input) -> bool:
    ...
    :func:`aristotle.agent.tools.execution.execute_tool_call` runs a synchronous gate
