...
and it is the reason ``aristotle.agent.model.client`` (Anthropic) and
``aristotle.agent.model.gemini_client`` (Gemini) can be interchanged by the CLI with
...
because that is what :mod:`aristotle.agent.runtime.runtime` constructs. Any
...
see :mod:`aristotle.agent.model.gemini_client` for a worked example of how much
...
:mod:`aristotle.agent.model.gemini_client`, and it is worth reading that module before
...
class ToolCall(TypedDict):
    ...
    happens in :func:`aristotle.agent.tools.execution.execute_tool_call`, which no
    ...
    Attributes:
        ...
        name: Must match a tool registered in a :class:`~aristotle.agent.tools.registry.ToolRegistry`;
