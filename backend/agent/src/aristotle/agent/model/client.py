"""Anthropic-backed implementation of :class:`~aristotle.agent.model.protocol.SendsMessages`.
...
Tool execution lives in :mod:`aristotle.agent.tools.execution`, which calls into this
...
class ModelClient:
    ...
    Args:
        ...
        max_retries: Additional attempts after the first, for
            :class:`~aristotle.agent.model.errors.ServingError` only.
    ...
    async def send(
        ...
        Args:
            messages: Anthropic-shaped message dicts, as built by
                :func:`aristotle.agent.context.assembly.assemble_context` and extended
                by the runtime's tool loop.
            tools: Model-facing schemas from
                :meth:`~aristotle.agent.tools.registry.ToolRegistry.schemas_for_run`.
    ...
    def _classify(exc: Exception) -> ModelLayerError:
        ...
        :class:`~aristotle.agent.model.errors.ContractError` because it is the
