...
effect still goes through :mod:`aristotle.agent.tools.execution` and its approval
gate.
...
from aristotle.agent.model.errors import ContractError
...
class OutputValidationError(ContractError):
    ...
    Subclasses :class:`~aristotle.agent.model.errors.ContractError` so that callers
