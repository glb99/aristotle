"""Gemini-backed implementation of :class:`~aristotle.agent.model.protocol.SendsMessages`.
...
change to :mod:`aristotle.agent.runtime.runtime`, the tool loop, or the session store
...
one (see :mod:`aristotle.agent.model.protocol`), a second provider does real
...
:mod:`~aristotle.agent.model.errors` taxonomy the Anthropic client uses. Those
...
from aristotle.agent.model.errors import (
    AssetError,
    ContractError,
    CredentialsError,
    ModelLayerError,
...
from aristotle.agent.model.protocol import ModelResponse, ToolCall
...
class GeminiClient:
    ...
    Args:
        ...
        max_retries: Additional attempts, :class:`~aristotle.agent.model.errors.ServingError` only.
    ...
    async def send(
        ...
        :meth:`aristotle.agent.model.client.ModelClient.send` and returns the same
        :class:`~aristotle.agent.model.protocol.ModelResponse`; the translation in
    ...
    def _to_model_response(response: Any) -> ModelResponse:
        ...
        :data:`~aristotle.agent.model.protocol.ToolCall` ``provider_data``, which the
