...
Not built:
    ...
    :mod:`aristotle.agent.session.store`. Nothing structural is in the way: this
...
from aristotle.agent.context.retrieval import Candidates, RecentMemory, RetrievesMemory
from aristotle.agent.session.store import MemoryEntry, SessionState
...
def assemble_context(
    ...
    Args:
        state: Session state, as returned by
            :meth:`~aristotle.agent.session.store.SessionStore.get_state`. Read-only
        ...
        retriever: Which rule picks them. Defaults to
            :class:`~aristotle.agent.context.retrieval.RecentMemory`, which is what
...
def _format_memory(chosen: dict[str, MemoryEntry]) -> str:
    ...
    :mod:`aristotle.agent.context.retrieval`'s question — this function used to answer
