...
This is the seam the persistence gap in :mod:`aristotle.agent.session.store` promised:
a durable store is *a second implementation of these five methods*, not a change
to any caller. :class:`~aristotle.agent.runtime.runtime.Runtime` is written against this
...
is. The dependency runs outward — aristotle declares the shape, whoever hosts it
...
- An unknown ``session_id`` raises
  :class:`~aristotle.agent.session.store.UnknownSessionError` rather than creating a
