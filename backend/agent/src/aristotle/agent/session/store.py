...
to a recency bound in :mod:`aristotle.agent.context.assembly` — there is no relevance
...
Not built:
    ...
    :class:`aristotle.agent.session.protocol.SessionRepository` is the complete set of
...
class SessionState:
    ...
    :func:`~aristotle.agent.context.assembly.assemble_context`.
    """
...
class MemoryRefused(Exception):
    ...
    *this handler is defective*, which is what
    :func:`~aristotle.agent.tools.execution.execute_tool_call` correctly does with
...
class SessionStore:
    ...
    **Why these methods are coroutines when none of them wait for anything.**
    ...
    async def propose(
        ...
        :func:`~aristotle.agent.context.assembly.assemble_context`, so an injected
