"""aristotle — a small agent built as infrastructure rather than as a script.
...
(:meth:`aristotle.agent.runtime.runtime.Runtime.run_turn`) and could have been written
...
- :mod:`aristotle.agent.interfaces` — receives work from outside; owns composition.
- :mod:`aristotle.agent.runtime` — sequences the turn; delegates every step.
- :mod:`aristotle.agent.context` — chooses what the model sees this turn.
- :mod:`aristotle.agent.model` — talks to a provider; proposes, never acts.
- :mod:`aristotle.agent.tools` — describes capabilities, gates them, runs them.
- :mod:`aristotle.agent.session` — the authoritative record; the only writer.
...
that only compute are not. So :mod:`aristotle.agent.model`, :mod:`aristotle.agent.session`,
:func:`aristotle.agent.tools.execution.execute_tool_call`, and
:meth:`aristotle.agent.runtime.runtime.Runtime.run_turn` are awaited, while
:mod:`aristotle.agent.context`, :mod:`aristotle.agent.tools.registry`, and the error and
...
point in :mod:`aristotle.agent.interfaces` — because a library that starts its own
