...
`aristotle.app.auth.service` gives at the top.
...
from aristotle.app.auth.dependencies import COOKIE_NAME, UNAUTHENTICATED, principal_for_key
from aristotle.app.auth.service import close_browser_session, open_browser_session
...
class KeyExchange(BaseModel):
    ...
    key: str = Field(min_length=1, description="An API key issued by `aristotle-admin issue-key`.")
