...
action, run from `aristotle.app.entrypoints.cli`.
...
from aristotle.app.auth.models import SESSION_LIFETIME, ApiKey, BrowserSession
from aristotle.app.auth.repository import ApiKeyRepository, BrowserSessionRepository
...
async def list_keys(session: AsyncSession, principal_id: Optional[str] = None) -> list[ApiKey]:
    ...
    **Also not an authorization check**, and less obviously so than
    ...
    That gap was found by hitting it. ``aristotle-admin chat`` refuses an unknown
...
async def open_browser_session(session: AsyncSession, principal_id: str) -> tuple[str, datetime]:
    ...
    **Takes a principal id, not a key**, and the signature is the guard. A
    ...
    :func:`~aristotle.app.auth.dependencies.principal_for_key` and passes what
