...
``aristotle`` imports this module, this package, or SQLModel.
...
from aristotle.app.sessions.memory import MemoryStore, TableMemoryStore
...
class SessionSummary:
    ...
    Deliberately not a :class:`~aristotle.agent.session.store.Session`. That type
...
def _default_store(session: AsyncSession) -> MemoryStore:
    ...
    **It does not choose.** Choosing is composition, which is what
    ...
    :func:`aristotle.app.personal.backing.configured_store` -- and handed in.
    """
...
class SqlSessionRepository:
    ...
    Satisfies :class:`aristotle.agent.session.protocol.SessionRepository` structurally.
