...
:mod:`aristotle.app.graph.catalogue` exists because of it.
...
:mod:`~aristotle.app.architecture.classify`. So the type system is split
...
`aristotle.app.core` imports two features today, which is the wart dialogue 14
...
A rename is exactly the claim ``same_as`` makes. ``aristotle.app.chat`` and
``aristotle.app.personal`` are one package under two names, both nodes keep
