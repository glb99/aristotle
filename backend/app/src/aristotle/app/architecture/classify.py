...
Everything in :mod:`~aristotle.app.architecture.derive` is given by the language:
...
from aristotle.app.architecture.catalogue import IS_A
from aristotle.app.architecture.derive import Derived
...
def propose(derived: Derived, *, threshold: int = RULE_OF_THREE) -> tuple[Proposal, ...]:
    ...
    **Nothing is proposed without positive evidence.** A package carrying the
    ...
    for package in sorted({module.package for module in derived.modules.values()}):
        ...
        # noise: `aristotle`, `aristotle.agent`, `app.alembic.versions`.
