...
``backend/app/src/aristotle`` holds only ``app`` and no ``__init__.py``, so
climbing yields ``.../src/aristotle`` as the root and every module loses its
``aristotle.`` prefix -- which silently drops **232 of 234 imports**, because
...
def python_files(root: Path) -> list[Path]:
    ...
    Shared with :func:`aristotle.app.architecture.derive.derive` rather than left
