...
retired. See :mod:`~aristotle.app.architecture.checks`.
...
from aristotle.app.architecture.catalogue import IMPORTS, MODULE, OWNS_TABLE, PACKAGE, TABLE
...
The names come from :mod:`~aristotle.app.architecture.catalogue` rather than
...
class Derived:
    ...
    what lets the boundaries in :mod:`~aristotle.app.architecture.checks` be
    ...
    def within(self, prefix: str) -> tuple[str, ...]:
        ...
        ``aristotle.app.core`` never accidentally matches ``aristotle.app.corex``,
...
def _resolve(target: str, known: frozenset[str]) -> str | None:
    ...
    its parent. ``from aristotle.app.graph.log import Assertion`` offers
