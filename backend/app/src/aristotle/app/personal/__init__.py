...
Contrast :mod:`~aristotle.app.architecture`, which shares the assertion table
...
Must not: reach into ``aristotle.agent.interfaces``. That package is the agent's
own composition root, for running it standalone. Composition for *this* process
happens in ``aristotle.app.entrypoints``. The direction of dependency is the
point — this package imports ``aristotle.agent``; the agent imports nothing from
