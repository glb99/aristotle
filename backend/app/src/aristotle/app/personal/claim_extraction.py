...
Not built:
    ...
    :func:`~aristotle.app.graph.service.revise` and nothing calls it from an
    ...
    *,
    ...
    Not built:
        **A second source.** A transcript is not the only thing worth extracting
        ...
        :func:`~aristotle.app.graph.service.observe` takes assertions,
        :func:`~aristotle.app.graph.service.refer_to` takes labels, and the
        ...
        - :class:`~aristotle.app.graph.models.GraphExtraction` keys its watermark
...
async def _to_assertion(
    ...
    *,
    ...
    Node ids come from :func:`~aristotle.app.graph.service.refer_to` rather than
...
def _interval(claim: dict[str, Any]) -> Interval:
    ...
    **A bound the supporting words do not carry is refused.** See
    :func:`~aristotle.app.personal.dates.stated_in`: a model that works a boundary out
...
def _side_fits(kind: str, expected: Optional[str]) -> bool:
    """``None`` accepts any kind — see :class:`~aristotle.app.graph.catalogue.Relation`."""
...
async def _advance(db: AsyncSession, session_id: str, through: int, now: datetime) -> None:
    ...
    two are not serialized — see :class:`~aristotle.app.graph.models.GraphExtraction`
