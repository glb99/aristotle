...
establishes who, :func:`~aristotle.app.sessions.access.load_owned_session` establishes
whether.
...
from aristotle.agent.session.store import SESSION_SCOPE, USER_SCOPE, MemoryScope
from aristotle.app.auth.dependencies import CurrentPrincipal
from aristotle.app.core.dependencies import AppSettings, DbSession
...
from aristotle.app.sessions.access import load_owned_session
...
async def list_sessions(principal: CurrentPrincipal, db: DbSession) -> list[SessionSummaryOut]:
    ...
    **The principal is the filter, and the client cannot influence it.** There
    ...
    """
    summaries = await session_repository(db).list_sessions(principal.id)
    return [
        SessionSummaryOut(
...
async def create_session(principal: CurrentPrincipal, db: DbSession) -> SessionCreated:
    ...
    """
    session = await session_repository(db).create_session(user_id=principal.id)
...
async def read_transcript(
    ...
    state = await load_owned_session(session_repository(db), principal, session_id)
    return [
        TranscriptEntry(
            kind=item.kind,
...
async def read_all_proposals(principal: CurrentPrincipal, db: DbSession) -> list[WaitingOut]:
    ...
    **No session id, so ownership is a filter rather than a check** -- the same
    ...
    """
    listing = await review.waiting_for(session_repository(db), principal.id)
    return [
        WaitingOut(
