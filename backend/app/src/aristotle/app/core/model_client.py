...
**Not the agent's own table.** ``aristotle.agent.interfaces`` has one, and it is
...
# Suppressed for the same reason as aristotle's own table: the annotation is the
...
lives in ``aristotle.agent.interfaces``, which is the agent's composition root for
