...
**Nothing here is imported by ``aristotle.agent``, and nothing in that package
...
transcript, and printed by ``aristotle-admin chat`` on its first line. It is not a
...
def _should_print_spans(console: bool, *, token: str, override: bool | None) -> bool:
    ...
    stdout is not a log — ``aristotle-admin`` relays what a model said — and no
...
def configure(service_name: str, *, console: bool = True) -> None:
    ...
    **No token means console output, not a refusal to boot**, and this is the one
    place the ``ARISTOTLE_`` prefix rule's "a typo refuses to start" behaviour
...
def _instrument_provider(provider: str) -> None:
    ...
    **This is how token counts and model latency arrive without
    ``aristotle.agent`` importing anything.** The agent calls these libraries;
...
async def job_span(
    ...
    Registered in :mod:`aristotle.app.core.jobs`, which owns the shape of a job.
