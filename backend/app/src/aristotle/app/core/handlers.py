...
:class:`~aristotle.app.core.protocols.Processable` and runs it when it applies.
...
merged even though both sequence steps. ``aristotle``'s runtime tracks step
...
async def call_either(fn: Callable[..., "_T | Awaitable[_T]"], *args: Any) -> _T:
    ...
    """
    # Same unnarrowable union as aristotle's `_call_either`: the branch proves
...
class StepHandler(Handler[DataType]):
    """A link that runs one :class:`~aristotle.app.core.protocols.Processable`.
