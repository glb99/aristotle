...
Values come from environment variables prefixed ``ARISTOTLE_`` and from a `.env`
...
Note the boundary with the agent. ``aristotle`` reads its own environment in its
...
class Settings(BaseSettings):
    ...
    Attributes:
        ...
        logfire_token: Where traces go. **Empty is not an error**, and that is
            the one exception to this class's "an unrecognised ``ARISTOTLE_``
        ...
        logfire_console: Whether spans are also printed to stdout. ``None``, the
            ...
            :func:`aristotle.app.core.observability._should_print_spans` for why
            ...
            It cannot put spans back into ``aristotle-admin``'s stdout. That
    ...
    model_config = SettingsConfigDict(
        ...
        # key needed to run ``uv run aristotle`` made this class refuse to
        # construct, and every route, task and test that touches settings failed
        # at once. Worse, the validation error printed the key.
        extra="ignore",
    )

    database_url: str = "postgresql+psycopg://aristotle:aristotle@localhost:5432/aristotle"
    log_level: str = "INFO"
    ...
    def _reject_unknown_prefixed_variables(self) -> "Settings":
        """Fail startup on a ``ARISTOTLE_*`` variable that matches no setting.
        ...
        The mistake being caught: ``ARISTOTLE_DATABSE_URL`` leaves ``database_url``
    ...
    def _warn_about_settings_missing_the_prefix(self) -> "Settings":
        """Warn when a setting's own name is set *without* ``ARISTOTLE_``.
        ...
        ``ARISTOTLE_RUN_WORKER_IN_APII`` refuses to boot in a second while
        ...
        checks and not an inconsistency. An ``ARISTOTLE_*`` variable matching no
...
def load_env_file() -> None:
    ...
    :class:`Settings` already reads `.env` for its own ``ARISTOTLE_*`` values, so
    ...
    keys by hand, while ``uv run aristotle`` worked — because the agent's own
