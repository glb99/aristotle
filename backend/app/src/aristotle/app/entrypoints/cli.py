...
into `aristotle.app.auth.service`, and prints the result; the decisions live there.
...
from aristotle.agent.session.store import (
    SESSION_SCOPE,
    USER_SCOPE,
    MemoryScope,
    UnknownSessionError,
)
from aristotle.app.architecture.checks import evaluate as check_boundaries
from aristotle.app.architecture.classify import propose, sentence
...
from aristotle.app.core import observability, platform
from aristotle.app.core.db import get_engine
from aristotle.app.core.jobs import register_tasks
from aristotle.app.core.settings import get_settings, load_env_file
from aristotle.app.evaluation.checks import Policy, evaluate
from aristotle.app.evaluation.runs import load_runs
from aristotle.app.graph.catalogue import PROMOTION_THRESHOLD, promotable
from aristotle.app.graph.repository import SqlGraphRepository, tally_relations
from aristotle.app.graph.service import expire_tail
from aristotle.app.personal import review
from aristotle.app.personal.backing import session_repository
from aristotle.app.personal.catalogue import VOCABULARY
from aristotle.app.personal.comparison import compare
from aristotle.app.personal.service import run_turn
from aristotle.app.sessions.models import ChatSession
...
async def _list_keys(principal_id: str | None) -> int:
    ...
    if not rows:
        ...
        print("`aristotle-admin issue-key <principal>` mints one.")
        return 0
...
async def _evaluate(
    ...
    :mod:`aristotle.app.evaluation.fixtures` for why those are not the same claim.
    """
    async with AsyncSession(get_engine()) as session:
...
async def _chat(principal_id: str, session_id: str | None) -> int:
    ...
    :class:`~aristotle.app.sessions.repository.SqlSessionRepository` the API builds,
    and calls the same :func:`~aristotle.app.personal.service.run_turn`. There is no
    ...
    Not the same as ``aristotle-agent``, and the difference is the point. That
    ...
    Extraction follows ``ARISTOTLE_MEMORY_EXTRACTION_ENABLED``, exactly as the
    route does. It only *enqueues*, so a worker has to be running for anything
    to come of it — ``just worker``, or ``ARISTOTLE_RUN_WORKER_IN_API`` on a
    ...
    async with queue.open_async(), AsyncSession(get_engine()) as db:
        ...
        if not await principal_is_known(db, principal_id):
            ...
            print("check the spelling, or `aristotle-admin issue-key <principal>` first.")
            return 1
...
async def _one_shot(handler, *args) -> int:
    ...
    that ``aristotle-admin list-proposals`` and ``/proposals`` are the same code
...
async def _relations(threshold: int) -> int:
    ...
    **A command rather than the periodic job ADR 0007 sketched**, and the reason
    ...
    print("\nPromote one by adding it to CATALOGUE in aristotle/app/graph/catalogue.py.")
    return 0
...
def main() -> int:
    ...
    observability.configure(service_name="aristotle-admin", console=False)

    parser = argparse.ArgumentParser(prog="aristotle-admin", description=__doc__)
