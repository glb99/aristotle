...
starts. See :mod:`aristotle.app.core.platform`.
...
def _configure() -> None:
    ...
    :func:`~aristotle.app.core.settings.load_env_file`.
    ...
    **This used to run at module scope, and that was a bug this module was
    ...
    Both were harmless while nobody had a `.env` with ``ARISTOTLE_*`` keys in it.
    ...
    observability.configure(service_name="aristotle-api")
    ...
    async with queue.open_async():
        if not settings.run_worker_in_api:
            if settings.memory_extraction_enabled:
                ...
                # aristotle-worker is running" and "nobody started one" are
                # indistinguishable. So it states the dependency instead of
                # diagnosing it.
                logger.info(
                    "memory extraction is enabled and no worker runs in this process; "
                    "a separate aristotle-worker must be running or proposals will never appear"
                )
            yield
            return
...
def main() -> int:
    ...
    :func:`aristotle.app.core.platform.run` is the only way to choose the loop.
    """
    import uvicorn
    ...
    config = uvicorn.Config(
        ...
        "aristotle.app.entrypoints.asgi:app",
