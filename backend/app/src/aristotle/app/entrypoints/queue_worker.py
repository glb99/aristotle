...
``ARISTOTLE_RUN_WORKER_IN_API`` starts a worker from the API's lifespan instead,
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(prog="aristotle-worker", description=__doc__)
    parser.add_argument(
        "--queue",
        action="append",
        dest="queues",
    ...
    # running the worker in-process reports `aristotle-api` for both, and the job
    # spans are still distinguishable by name -- which is the shape of the
    # answer there.
    observability.configure(service_name="aristotle-worker")
    try:
