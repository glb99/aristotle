...
from aristotle.app.graph.catalogue import Relation, read_as
from aristotle.app.graph.conclusions import Conclusion, stale_after
from aristotle.app.graph.constraints import Conflict, conflicts_for
from aristotle.app.graph.identity import SELF, Node, normalize, owner_node_id
from aristotle.app.graph.inference import infer_succession
from aristotle.app.graph.log import Assertion, log_expire
from aristotle.app.graph.log import retract as log_retract
from aristotle.app.graph.repository import SqlGraphRepository, UnknownNodeError
...
async def refer_to(
    ...
    :mod:`aristotle.app.graph.identity` for why the conservative direction is the
...
def _unassumed(assertions: Sequence[Assertion], believed: Sequence[Assertion]) -> list[Assertion]:
    ...
    **The signature is exact and needs no language at all**: a claim whose
    ...
    performing one is :func:`~aristotle.app.graph.inference.infer_succession`'s
