...
Everything above it — :mod:`~aristotle.app.graph.temporal`,
:mod:`~aristotle.app.graph.constraints`, :mod:`~aristotle.app.graph.inference` —
...
the rows themselves. Handing back a :class:`~aristotle.app.graph.models.GraphAssertion`
...
from aristotle.app.graph.catalogue import EMPTY, Vocabulary
from aristotle.app.graph.conclusions import Conclusion
from aristotle.app.graph.identity import Node, normalize
from aristotle.app.graph.log import Assertion
from aristotle.app.graph.models import (
    GraphAssertion,
    GraphConclusion,
    GraphConclusionEvidence,
    GraphNode,
)
from aristotle.app.graph.temporal import Interval
...
def _as_utc(value: datetime) -> datetime:
    ...
    and every comparison in :mod:`~aristotle.app.graph.temporal` is a comparison.
    """
...
class SqlGraphRepository:
    ...
    async def believed_at(self, user_id: str, moment: datetime) -> list[Assertion]:
        ...
        :meth:`~aristotle.app.graph.log.Assertion.believed_at`, because the log
    ...
    async def supersede(self, closed: Assertion, replacement: Assertion) -> None:
        ...
        Takes the pair :func:`~aristotle.app.graph.log.supersede` produced rather
